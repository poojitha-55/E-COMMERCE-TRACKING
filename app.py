...
with col1:
    st.markdown(
        ...
        <div style="font-size: 40px; text-align: left; width: 100%;">
            🛒🛍️💡 <strong>E-Commerce Competitor Strategy Dashboard</strong> 💡🛍️🛒
        </div>
        """,
...
with col2:
    st.markdown(
        ...
        <div style="text-align: right;">
            <img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExNzhxb2d1bHhkZ25pZHZ3M3c0aXZienl3cWlqNGlrbDZkZXF3dW1yYyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/nNOAPjUdo4mpZFkDf8/giphy.webp" alt="Engaging GIF" width="300">
...
def load_competitor_data():
    ...
    st.write(data)  # Display the full dataset instead of just the first 5 rows
    return data
...
def generate_strategy_recommendation(product_name, competitor_data, sentiment):
    ...
    *Today's Date*: {str(date)}
    ...
    # Debug: print the response to check the structure
    print("API Response:", res.text)

    try:
        res = res.json()  # Ensure it's a JSON response
        # Check if 'choices' key exists
        if 'choices' in res and len(res['choices']) > 0:
            response = res["choices"][0]["message"]["content"]
            return response
        else:
            st.write("Error: 'choices' key missing in API response.")
            return "No response from the model. Please check the API request."
    except KeyError as e:
        st.write(f"Error parsing API response: {e}")
        return "Error occurred while parsing the response."
    except json.JSONDecodeError:
        st.write("Error decoding the API response.")
        return "Error occurred while decoding the response."

# Streamlit UI

st.sidebar.header("🛍️Select a Product")
